...
label request_Action(Girl, Action_type):
    ...
    if approval:
        if Girl.History.check(Action_type, tracker = "recent"):
            call recent_Action_reactions(Girl, Action_type)

            $ accepted = True
        elif Girl.History.check(Action_type, tracker = "daily"):
            call daily_Action_reactions(Girl, Action_type)

            $ accepted = True

    if not accepted:
        if not Girl.History.check(Action_type) and not Girl.History.check(f"no_{Action_type}"):
            call first_time_asking_reactions(Girl, Action_type)

        if approval:
            if not Girl.History.check(Action_type):
                call first_approval_reactions(Girl, Action_type)
            else:
...
label stop_all_Actions:
    ...
    while temp_Girls:
        ...
        if renpy.showing(f"{temp_Girls[0].tag}_sprite"):
            call show_Character(temp_Girls[0])
