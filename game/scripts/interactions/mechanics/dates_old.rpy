label Date_Ask(Girl=0):
    ...
    menu:
        "Good, I'll meet you in the campus square." if bg_current != "bg campus" or Time_Count < 2:
            $ Girl.FaceChange("smile")
        "Good, let's get going then." if bg_current == "bg campus" and Time_Count == 2:
            $ Girl.FaceChange("smile")
    ...
    if bg_current != "bg campus" or Time_Count < 2:
        if Girl == RogueX:
...
label DateNight(Date_Bonus=[0,0], Play_Cost=0, Date_Cost=[0,0], BO=[]):
    ...
    $ BO = ActiveGirls[:]
    while BO:
        if "yesdate" in BO[0].DailyActions:
            $ Party.append(BO[0])
